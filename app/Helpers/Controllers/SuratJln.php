        ...
        if ($request->getMethod(true) == 'POST') {
            ...
            foreach ($SuratJln as $sl) {
                ...
                if (session()->get('level') == "1") {
                    $row[] =
                        '<a href="' . base_url('SuratJln/edit/' . $sl['id_sj']) . '" class="btn btn-success btn-xs mr-2"></i>Edit</a>' .
                        '<a href="' . base_url('SuratJln/detail/' . $sl['id_sj']) . '" class="btn btn-info btn-xs mr-2">Detail</a>' .
                        '<a href="' . base_url('SuratJln/printsj/' . $sl['id_sj']) . '" class="btn btn-warning btn-xs mr-2">Print</a>' .
                        "<button type=\"button\" class=\"btn btn-danger btn-xs\" onclick=\"hapusSuratJln('" . $sl['no_suratjln'] . "','" . $sl['nama_customer'] . "') \">Delete</button>";
                } else {
                    $row[] =
                        '<a href="' . base_url('SuratJln/detail/' . $sl['id_sj']) . '" class="btn btn-info btn-xs mr-2">See Detail</a>' .
                        '<a href="' . base_url('SuratJln/printsj/' . $sl['id_sj']) . '" class="btn btn-danger btn-xs mr-2">Print DO</a>';
                }
                $data[] = $row;
            }
            $output = [
