    ...
    public function listData()
    {
        ...
        if ($request->getMethod(true) == 'POST') {
            ...
            foreach ($barang as $brg) {
                ...
                $row[] =
                    '<a href="' . base_url('barang/edit/' . $brg['id_barang']) . '" class="btn btn-success btn-xs mr-2" style="height:20px;font-size: 11px;">EDIT</a>' .
                    "<button type=\"button\" class=\"btn btn-danger btn-xs\" style=\"height:20px;font-size: 11px;\" onclick=\"hapusBarang('" . $brg['id_barang'] .  "',`" . $brg['nama_barang'] . "`) \" >DELETE</button>";            
                $data[] = $row;
            }
            $output = [
    ...
    public function hapus()
    {
        if ($this->request->isAJAX()) {
            ...
            $datalog = [
                'username'  => session()->get('username'),
                'jamtrx'    => date('Y-m-d H:i:s'),
                'kegiatan'  => 'Meghapus Tabel Barang : ' . $nama_barang,
            ];
            $this->ModelLogHistory->add($datalog);
