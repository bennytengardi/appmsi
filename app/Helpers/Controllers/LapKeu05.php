...
use App\Models\ModelLapKeu05;
use Config\Services;

use PhpOffice\PhpSpreadsheet\Spreadsheet;
use PhpOffice\PhpSpreadsheet\Writer\Xlsx;

class LapKeu05 extends BaseController
{
    function __construct()
    {
        $this->ModelAccount  = new ModelAccount();
        $this->ModelJurnal   = new ModelJurnal();
        $this->ModelLapKeu05 = new ModelLapKeu05();
    }
    ...
    public function index()
    {
        ...
        return view('lapkeu05/filter', $data);
    }
    ...
    public function preview()
    {
        ...
        $awl = $this->ModelLapKeu05->Neraca($sampai);
        ...
        foreach ($awl as $aw1) {
            ...
            $this->ModelLapKeu05->updateSaldo($raw1);
        }
        ...
        $tgl4 = date('Y-m-d', $tg4);
        $tgl3 = date('Y-m-d', $tg3);
        $rllalu = $this->ModelLapKeu05->rllalu($tgl3, $tgl4);
        $rlini  = $this->ModelLapKeu05->rlini($tgl4, $sampai);
        ...
        $this->ModelLapKeu05->updateSaldo($data);

        $data1 = [
            'crd' => $rlini[0],
            'kode_account' => $acct2
        ];
        $this->ModelLapKeu05->updateSaldo($data1);

        $data1 = $this->ModelLapKeu05->Balance();
        $this->ModelLapKeu05->clearNeraca();
        ...
        foreach ($data1 as $dt1) {
            if ($dt1['sub_account'] != null) {
                ...
                $data = [
                    'kode_account' => $dt1['kode_account'],
                    'saldoawal'    => $dt1['sldawal'],
                    'trxdebet'     => $dt1['slddbt'],
                    'trxcredit'    => $dt1['sldcrd'],
                    'rectype'      => $rec
                ];
                $this->ModelLapKeu05->insertNeraca($data);
            }
        }

        $laporan = $this->ModelLapKeu05->saldoNeraca();
        ...
        if (isset($tombolCetak)) {
            ...
            return view('lapkeu05/tampil', $data);
        }
