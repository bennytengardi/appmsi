    ...
    public function listData()
    {
        ...
        if ($request->getMethod(true) == 'POST') {
            ...
            foreach ($purchinv as $purch) {
                ...
                $row[] = $purch['currency'];
                $row[] = number_format($purch['kurs'],'2',',','.');
                $row[] = number_format($purch['total_invoice'], '2', ',', '.');
                $row[] = number_format($purch['total_bayar'], '2', ',', '.');
                if (session()->get('level') == "1") {
                    $row[] =
                        '<a href="' . base_url('PurchInv/edit/' . $purch['no_invoice']) . '" class="btn btn-success btn-xs mr-2"></i>EDIT</a>' .
                        '<a href="' . base_url('PurchInv/detail/' . $purch['no_invoice']) . '" class="btn btn-info btn-xs mr-2">DETAIL</a>' .
                        '<a href="' . base_url('PurchInv/print/' . $purch['no_invoice']) . '" class="btn btn-primary btn-xs mr-2">PRINT</a>' .
                        "<button type=\"button\" class=\"btn btn-danger btn-xs\" onclick=\"hapusPurchInv('" . $purch['no_invoice'] . "','" . $purch['nama_supplier'] . "') \">DELETE</button>";
                } else {
                    $row[] =
                        '<a href="' . base_url('PurchInv/detail/' . $purch['no_invoice']) . '" class="btn btn-info btn-xs mr-2">DETAIL</a>';
                }
    ...
    public function simpandata()
    {
        if ($this->request->isAJAX()) {
            ...
            if (!$valid) {
                ...
                $ambildatakeranjang = $this->ModelPurchInv->ambilDataCart();
                if (empty($ambildatakeranjang)) {
                    ...
                    $datalog = [
                        ...
                        'kegiatan'  => 'Menambah Invoice Pembelian  : ' . $no_invoice,
                    ];
        ...
        if ($this->request->isAJAX()) {
            ...
            if (empty($ambildatakeranjang)) {
                ...
                $datalog = [
                    'username'  => session()->get('username'),
                    'jamtrx'    => date('Y-m-d H:i:s'),
                    'kegiatan'  => 'Merubah Invoice Pembelian  : ' . $no_invoice,
                ];
                $this->ModelLogHistory->add($datalog);
    ...
    public function hapus()
    {
        if ($this->request->isAJAX()) {
            ...
            $datalog = [
                'username'  => session()->get('username'),
                'jamtrx'    => date('Y-m-d H:i:s'),
                'kegiatan'  => 'Menghapus Invoice Pembelian  : ' . $noinvoice,
            ];
            $this->ModelLogHistory->add($datalog);
