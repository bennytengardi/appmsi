    ...
    public function listData()
    {
        ...
        if ($request->getMethod(true) == 'POST') {
            ...
            foreach ($payment as $cr) {
                ...
                $row[] =
                    '<a href="' . base_url('Payment/edit/' . $cr['no_payment']) . '" class="btn btn-success btn-xs mr-2"  style="font-size: 12px"><i class="fa fa-edit"></i> Edit</a>' .
                    '<a href="' . base_url('Payment/detail/' . $cr['no_payment']) . '" class="btn btn-info btn-xs mr-2"  style="font-size: 12px"><i class="fa fa-eye"></i> Detail</a>' .
                    "<button type=\"button\" class=\"btn btn-danger btn-xs\"  style=\"font-size: 12px;\" onclick=\"hapusPayment('" . $cr['no_payment'] . "','" . $cr['nama_supplier'] . "') \"> <i class='fa fa-trash-alt'></i> Delete</button>";
                $data[] = $row;
            }
            $output = [
            ...
            if (!$valid) {
               ...
               if ($total_ongkir <> 0) {
                ...
                $datalog = [
                    ...
                    'kegiatan'  => 'Menambah Pembayaran Hutang : ' . $no_payment,
                ];
        ...
        if ($this->request->isAJAX()) {
           ...
           if ($total_pph23 <> 0) {
            ...
            $datalog = [
                'username'  => session()->get('username'),
                'jamtrx'    => date('Y-m-d H:i:s'),
                'kegiatan'  => 'Merubah Pembayaran Hutang : ' . $no_payment,
            ];
            $this->ModelLogHistory->add($datalog);
    ...
    public function hapus()
    {
        if ($this->request->isAJAX()) {
            ...
            $datalog = [
                'username'  => session()->get('username'),
                'jamtrx'    => date('Y-m-d H:i:s'),
                'kegiatan'  => 'Menghapus Pembayaran Hutang : ' . $no_payment,
            ];
            $this->ModelLogHistory->add($datalog);
