            ...
            <table class="table table-bordered table-sm" width="100%">
                <thead class="bg-primary">
                    <tr class="text-center">
                        <td width="4%">NO</td>
                        <td width="18%">NO INVOICE</td>
                        <td width="6%">DATE</td>
                        <td width="12%">TOTAL INVOICE</td>
                    ...
                    ?>
                        <tr>
                            ...
                            <td style="text-align: center;">
                                ...
                                <button class="btn btn-xs btn-success ml-2" onclick="edititem('<?= $row['no_invoice'] ?>','<?= $row['tgl_invoice'] ?>','<?= $row['total_invoice'] ?>','<?= $row['total_bayar'] ?>')">EDIT</button>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                ...
                <tr>
                    ...
                    <td>
                        <input type="text" id="total_saldo" name="total_saldo" class="form-control form-control-sm border-secondary text-right" value="<?= number_format($total_saldo, 0) ?>" style="height: 26px" readonly>
...
<script>
    function edititem(noinvoice, tginvoice, total, totalbyr) {
        ...
        $.ajax({
            type: "post",
            url: "<?= site_url('supplier/viewEditHarga') ?>",
            data: {
    ...
    function hapusitem(noinvoice) {
        ...
        Swal.fire({
            ...
            if (result.value) {
                $.ajax({
                    type: "post",
                    url: "<?= base_url('supplier/hapusItem') ?>",
                    data: {
                        no_invoice: no_invoice
                    },
                    dataType: "json",
