...
class OthRcv extends BaseController
{
    ...
    public function listData()
    {
        ...
        if ($request->getMethod(true) == 'POST') {
            ...
            foreach ($othrcv as $cr) {
                ...
                $row[] = $cr['keterangan'];
                $row[] =
                    '<a href="' . base_url('OthRcv/edit/' . $cr['id_bukti']) . '" class="btn btn-success btn-xs mr-2" style="font-size: 10px;height: 18px">Edit</a>' .
                    '<a href="' . base_url('OthRcv/detail/' . $cr['id_bukti']) . '" class="btn btn-info btn-xs mr-2" style="font-size: 10px;height: 18px">Detail</a>' .
                    '<a href="' . base_url('OthRcv/print/' . $cr['id_bukti']) . '" class="btn btn-primary btn-xs mr-2" style="font-size: 10px;height: 18px">Print</a>' .
                    "<button type=\"button\" class=\"btn btn-danger btn-xs\"  style=\"font-size: 10px;height: 18px\" onclick=\"hapusOthRcv('" . $cr['id_bukti'] . "','" . $cr['no_bukti'] . "') \">Delete</button>";
                $data[] = $row;
            }
            $output = [
        ...
        if ($this->request->isAJAX()) {
            ...
            if ($request->getMethod(true) == 'POST') {
                ...
                foreach ($lists as $list) {
                    ...
                    $row[] = "<button type=\"button\" class=\"btn btn-sm btn-primary align-middle\" style=\"height: 18px; font-size: 10px;\"  onclick=\"pilihitem('" . $list['kode_account'] . "','" . $list['nama_account'] . "')\" ><i class=\"fa fa-check\"></i></button>";
                    $data[] = $row;
                }
                $output = [
                    "draw" => $request->getPost('draw'),
            ...
            if (!$valid) {
                ...
                if (empty($ambildatakeranjang)) {
                    ...
                    $datalog = [
                        ...
                        'kegiatan'  => 'Menambah Bukti Penerimaan Kas/Bank : ' . $no_bukti,
                    ];
    ...
    public function updatedata()
    {
        if ($this->request->isAJAX()) {
            ...
            $ambildatakeranjang = $this->ModelOthRcv->ambilCart($no_bukti);
            if (empty($ambildatakeranjang)) {
                $msg = [
                    'error' => 'Maaf, Belum diisi transaksi !!!!'
                ];
                ...
                foreach ($ambildatakeranjang as $row) :
                    ...
                    if ($stt == 'A') {
                     ...
                     if ($stt == 'D') {
                        $this->ModelOthRcv->deleteDetailOthRcv($row_id);
                     } 
                     if ($stt == 'E') {
                       $this->ModelOthRcv->updateDetailOthRcv($datapay);
                     }
                    $data4 = [
                        'no_voucher'     => $no_bukti,
                        'tgl_voucher'    => $tgl_bukti,
                ...
                $datalog = [
                    'username'  => session()->get('username'),
                    'jamtrx'    => date('Y-m-d H:i:s'),
                    'kegiatan'  => 'Merubah Bukti Penerimaan Kas/Bank : ' . $no_bukti,
                ];
                $this->ModelLogHistory->add($datalog);
    ...
    public function hapus()
    {
        if ($this->request->isAJAX()) {
            ...
            $datalog = [
                'username'  => session()->get('username'),
                'jamtrx'    => date('Y-m-d H:i:s'),
                'kegiatan'  => 'Menghapus Bukti Penerimaan Kas/Bank : ' . $nobukti,
            ];
            $this->ModelLogHistory->add($datalog);
