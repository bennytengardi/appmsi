    ...
    public function listData()
    {
        ...
        if ($request->getMethod(true) == 'POST') {
            ...
            foreach ($supplier as $cus) {
                ...
                $row[] = substr($cus['address1'],0,40);
                $row[] = $cus['telephone'];
                $row[] = $cus['currency'];
                $row[] = number_format($cus['awal'] + $cus['debet'] - $cus['credit'], 0, '.', ',');
                $row[] =
                    '<a href="' . base_url('supplier/edit/' . $cus['kode_supplier']) . '" class="btn btn-success btn-xs mr-1" style="font-size: 10px;height: 18px;">EDIT</a>' .
                    "<button type=\"button\" class=\"btn btn-danger btn-xs\" style=\"font-size: 10px;height: 18px;\"  onclick=\"hapusSupplier('" . $cus['kode_supplier'] .  "','" . $cus['nama_supplier'] . "') \">DELETE</button>" .
                    '<a href="' . base_url('supplier/saldoawal/' . $cus['kode_supplier']) . '" class="btn btn-info btn-xs ml-1" style="font-size: 10px;height: 18px;">SALDO AWAL</a>';
                $data[] = $row;
            }
            $output = [
