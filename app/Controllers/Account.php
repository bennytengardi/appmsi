    ...
    public function listData()
    {
        ...
        if ($request->getMethod(true) == 'POST') {
            ...
            foreach ($account as $acct) {
                ...
                $row[] =
                    '<a href="' . base_url('account/edit/' . $acct['kode_account']) . '" class="btn btn-success btn-xs mr-2" style="font-size: 10px;height: 18px;">Edit</a>' .
                    "<button type=\"button\" class=\"btn btn-danger btn-xs\"  style=\"font-size: 10px;height: 18px;\" onclick=\"hapusAccount('" . $acct['kode_account'] .  "','" . $acct['nama_account'] . "') \">Delete</button>";
                $data[] = $row;
            }
            $output = [
        ...
        if ($this->request->isAJAX()) {
            ...
            if (!$valid) {
                ...
                $datalog = [
                    ...
                    'kegiatan'  => 'Menambah Tabel Account : ' . $nama_account,
                ];
        ...
        if ($this->request->isAJAX()) {
            ...
            if (!$valid) {
                ...
                $datalog = [
                    'username'  => session()->get('username'),
                    'jamtrx'    => date('Y-m-d H:i:s'),
                    'kegiatan'  => 'Merubah Tabel Account : ' . $nama_account,
                ];
                $this->ModelLogHistory->add($datalog);
    ...
    public function hapus()
    {
        if ($this->request->isAJAX()) {
            ...
            $datalog = [
                'username'  => session()->get('username'),
                'jamtrx'    => date('Y-m-d H:i:s'),
                'kegiatan'  => 'Menghapus Tabel Account : ' . $nama_account,
            ];
            $this->ModelLogHistory->add($datalog);
