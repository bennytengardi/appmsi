    ...
    public function listData()
    {
        ...
        if ($request->getMethod(true) == 'POST') {
            ...
            foreach ($receipt as $cr) {
                ...
                $row[] =
                    '<a href="' . base_url('Receipt/edit/' . $cr['no_receipt']) . '" class="btn btn-success btn-xs mr-2"  style="font-size: 10px;height: 18px;"><i class="fa fa-edit"></i> Edit</a>' .
                    '<a href="' . base_url('Receipt/detail/' . $cr['no_receipt']) . '" class="btn btn-info btn-xs mr-2"  style="font-size: 10px;height: 18px;"><i class="fa fa-eye"></i> Detail</a>' .
                    "<button type=\"button\" class=\"btn btn-danger btn-xs\"  style=\"font-size: 10px;height: 18px;\" onclick=\"hapusReceipt('" . $cr['no_receipt'] . "','" . $cr['nama_customer'] . "') \"> <i class='fa fa-trash-alt'></i> Delete</button>";
                $data[] = $row;
            }
            $output = [
        ...
        if ($this->request->isAJAX()) {
            ...
            if (!$valid) {
                ...
                $datalog = [
                    ...
                    'kegiatan'  => 'Menambah Pelunasan Customer : ' . $no_receipt,
                ];
    ...
    public function updatedata()
    {
        if ($this->request->isAJAX()) {
            ...
            $datalog = [
                'username'  => session()->get('username'),
                'jamtrx'    => date('Y-m-d H:i:s'),
                'kegiatan'  => 'Merubah Pelunasan Customer : ' . $no_receipt,
            ];
            $this->ModelLogHistory->add($datalog);
    ...
    public function hapus()
    {
        if ($this->request->isAJAX()) {
            ...
            $datalog = [
                'username'  => session()->get('username'),
                'jamtrx'    => date('Y-m-d H:i:s'),
                'kegiatan'  => 'Menghapus Pelunasan Customer : ' . $no_receipt,
            ];
            $this->ModelLogHistory->add($datalog);
