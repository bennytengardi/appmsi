    ...
    public function listData()
    {
        ...
        if ($request->getMethod(true) == 'POST') {
            ...
            foreach ($customer as $cus) {
                ...
                $row[] = substr($cus['address1'],0,50);
                $row[] = $cus['telephone'];
                $row[] = number_format($cus['awal'] + $cus['debet'] - $cus['credit'], 0, '.', ',');
                $row[] =
                    '<a href="' . base_url('customer/edit/' . $cus['kode_customer']) . '" class="btn btn-success btn-xs mr-2" style="font-size:10px;height: 18px;">EDIT</a>' .
                    "<button type=\"button\" class=\"btn btn-danger btn-xs\" style=\"font-size:10px;height: 18px;\" onclick=\"hapusCustomer('" . $cus['kode_customer'] .  "','" . $cus['nama_customer'] . "') \">DELETE</button>" .
                    '<a href="' . base_url('customer/saldoawal/' . $cus['kode_customer']) . '" class="btn btn-info btn-xs ml-1" style="font-size:10px;height: 18px;">SALDO AWAL</a>';
                $data[] = $row;
            }
            $output = [
