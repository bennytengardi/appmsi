        ...
        if ($request->getMethod(true) == 'POST') {
            ...
            foreach ($salesinv as $sl) {
                ...
                if (session()->get('level') == "1") {
                    $row[] =
                        '<a href="' . base_url('SalesInv/edit/' . $sl['id_inv']) . '" class="btn btn-success btn-xs mr-2" style="font-size: 10px;height: 18px;"></i>Edit</a>' .
                        '<a href="' . base_url('SalesInv/detail/' . $sl['id_inv']) . '" class="btn btn-info btn-xs mr-2" style="font-size: 10px;height: 18px;">Detail</a>' .
                        '<a href="' . base_url('SalesInv/print/' . $sl['id_inv']) . '" class="btn btn-primary btn-xs mr-2" style="font-size: 10px;height: 18px;">Inv Jhns</a>' .
                        '<a href="' . base_url('SalesInv/print2/' . $sl['id_inv']) . '" class="btn btn-primary btn-xs mr-2" style="font-size: 10px;height: 18px;">Inv Frnk</a>' .
                        "<button type=\"button\" class=\"btn btn-danger btn-xs\" style=\"font-size: 10px;height: 18px;\" onclick=\"hapusSalesInv('" . $sl['no_invoice'] . "','" . $sl['nama_customer'] . "') \">Delete</button>";
                } else {
                    $row[] =
                        '<a href="' . base_url('SalesInv/detail/' . $sl['id_inv']) . '" class="btn btn-info btn-xs mr-2" style="font-size: 10px;height: 18px;">See Detail</a>' .
                        '<a href="' . base_url('SalesInv/print/' . $sl['id_inv']) . '" class="btn btn-success btn-xs mr-2" style="font-size: 10px;height: 18px;">Inv Johnson</a>' .
                        '<a href="' . base_url('SalesInv/print2/' . $sl['id_inv']) . '" class="btn btn-danger btn-xs mr-2" style="font-size: 10px;height: 18px;">Inv Franklin</a>' ;
                }
                $data[] = $row;
            }
            $output = [
