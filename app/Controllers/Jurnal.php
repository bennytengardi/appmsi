    ...
    public function listData()
    {
        ...
        if ($request->getMethod(true) == 'POST') {
            ...
            foreach ($jurnal as $jur) {
                ...
                $row[] = '';
                $row[] =
                    '<a href="' . base_url('jurnal/edit/' . $jur['no_voucher']) . '" class="btn btn-success btn-xs mr-2" style="font-size: 10px;height: 18px;"></i>Edit</a>' .
                    '<a href="' . base_url('jurnal/detail/' . $jur['no_voucher']) . '" class="btn btn-info btn-xs mr-2" style="font-size: 10px;height: 18px;"> See Detail</a>' .
                    "<button type=\"button\" class=\"btn btn-danger btn-xs\" style=\"font-size: 10px;height: 18px;\" onclick=\"hapusJurnal('" . $jur['no_voucher'] . "') \">Delete</button>";
                $data[] = $row;
            }
            $output = [
