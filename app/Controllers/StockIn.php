...
class StockIn extends BaseController
{
        ...
        if ($request->getMethod(true) == 'POST') {
            ...
            foreach ($stockin as $stokin) {
                ...
                if (session()->get('level') == "1") {
                    $row[] =
                        '<a href="' . base_url('StockIn/edit/' . $stokin['no_bukti']) . '" class="btn btn-success btn-xs mr-2" style="font-size: 10px;height: 18px;"></i>Edit</a>' .
                        '<a href="' . base_url('StockIn/detail/' . $stokin['no_bukti']) . '" class="btn btn-info btn-xs mr-2" style="font-size: 10px;height: 18px;">Detail</a>' .
                        "<button type=\"button\" class=\"btn btn-danger btn-xs\" style=\"font-size: 10px;height: 18px;\" onclick=\"hapusStockIn('" . $stokin['no_bukti'] . "','" . $stokin['nama_supplier'] . "') \">Delete</button>";
                } else {
                    $row[] =
                        '<a href="' . base_url('StockIn/detail/' . $stokin['no_bukti']) . '" class="btn btn-info btn-xs mr-2" style="font-size: 10px;height: 18px;">See Detail</a>';
                }
                $data[] = $row;
            }
            $output = [
    ...
    public function add()
    {
        ...
        $this->ModelStockIn->clearCart($no_bukti);
    ...
    public function simpandata()
    {
        if ($this->request->isAJAX()) {
            ...
            if (!$valid) {
                ...
                $ambildatakeranjang = $this->ModelStockIn->ambilCart($no_bukti);
                if (empty($ambildatakeranjang)) {
                    ...
                    $datalog = [
                        ...
                        'kegiatan'  => 'Menambah Pemasukan Barang : ' . $no_bukti,
                    ];
        ...
        if ($this->request->isAJAX()) {
            ...
            if (empty($ambildatakeranjang)) {
                ...
                $datalog = [
                    'username'  => session()->get('username'),
                    'jamtrx'    => date('Y-m-d H:i:s'),
                    'kegiatan'  => 'Merubah Pemasukan Barang : ' . $no_bukti,
                ];
                $this->ModelLogHistory->add($datalog);
    ...
    public function hapus()
    {
        if ($this->request->isAJAX()) {
            ...
            $datalog = [
                'username'  => session()->get('username'),
                'jamtrx'    => date('Y-m-d H:i:s'),
                'kegiatan'  => 'Menghapus Pemasukan Barang : ' . $nobukti,
            ];
            $this->ModelLogHistory->add($datalog);
    ...
    {
        $kode_supplier = $this->request->getPost('kode_supplier');
        $data = $this->ModelStockIn->ambilDataPO($kode_supplier);
    ...
    public function get_datapo()
    {
        if ($this->request->isAJAX()) {
            ...
            $no_bukti = $this->request->getPost('no_bukti');
            $this->ModelStockIn->clearCart($no_bukti);
            $datapo = $this->ModelStockIn->get_pobybrg($no_po);
            foreach ($datapo as $list) {
                ...
                $this->ModelStockIn->addCart($isikeranjang);
            }
            $msg = ['sukses' => 'berhasil'];
            echo json_encode($msg);
        }
