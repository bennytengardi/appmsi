        ...
        if ($request->getMethod(true) == 'POST') {
            ...
            foreach ($salesord as $sl) {
                ...
                if (session()->get('level') == "1") {
                    $row[] =
                        '<a href="' . base_url('SalesOrd/edit/' . $sl['id_so']) . '" class="btn btn-success btn-xs mr-2" style="font-size: 10px;height: 18px;"></i>Edit</a>' .
                        '<a href="' . base_url('SalesOrd/detail/' . $sl['id_so']) . '" class="btn btn-info btn-xs mr-2" style="font-size: 10px;height: 18px;">Detail</a>' .
                        '<a href="' . base_url('SalesOrd/print/' . $sl['id_so']) . '" class="btn btn-primary btn-xs mr-2" style="font-size: 10px;height: 18px;">Print</a>' .
                        "<button type=\"button\" class=\"btn btn-danger btn-xs\" style=\"font-size: 10px;height: 18px;\" onclick=\"hapusSalesOrd('" . $sl['no_so'] . "','" . $sl['nama_customer'] . "') \">Delete</button>";
                } else {
                    $row[] =
                        '<a href="' . base_url('SalesOrd/detail/' . $sl['id_so']) . '" class="btn btn-info btn-xs mr-2" style="font-size: 10px;height: 18px;">Detail</a>' .
                        '<a href="' . base_url('SalesOrd/print/' . $sl['id_so']) . '" class="btn btn-success btn-xs mr-2" style="font-size: 10px;height: 18px;">Print</a>'  ;
                }
                $data[] = $row;
            }
            $output = [
        ...
        if ($this->request->isAJAX()) {
            ...
            if ($request->getMethod(true) == 'POST') {
                ...
                foreach ($lists as $list) {
                    ...
                    $row[] = "<button type=\"button\" class=\"btn btn-xs btn-info\" style=\"height: 18px; font-size: 10px;\"  onclick=\"pilihitem('" . $list['id_barang'] .  "','" . "')\" ><i class=\"fa fa-check\"></i></button>";
                    $data[] = $row;
                }
                $output = [
                    "draw" => $request->getPost('draw'),
