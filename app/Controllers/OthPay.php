...
class OthPay extends BaseController
{
    ...
    public function listData()
    {
        ...
        if ($request->getMethod(true) == 'POST') {
            ...
            foreach ($othpay as $cr) {
                ...
                $row[] = $cr['keterangan'];
                $row[] =
                    '<a href="' . base_url('OthPay/edit/' . $cr['id_bukti']) . '" class="btn btn-success btn-xs mr-2" style="height: 18px;font-size:10px;">Edit</a>' .
                    '<a href="' . base_url('OthPay/detail/' . $cr['id_bukti']) . '" class="btn btn-info btn-xs mr-2" style="height: 18px;font-size:10px;">Detail</a>' .
                    '<a href="' . base_url('OthPay/print/' . $cr['id_bukti']) . '" class="btn btn-primary btn-xs mr-2" style="height: 18px;font-size:10px;"> Print</a>' .
                    "<button type=\"button\" class=\"btn btn-danger btn-xs\" style=\"height: 18px;font-size:10px;\" onclick=\"hapusOthPay('" . $cr['id_bukti'] . "','" . $cr['no_bukti'] . "') \">Delete</button>";
                $data[] = $row;
            }
            $output = [
            ...
            if (!$valid) {
                ...
                if (empty($ambildatakeranjang)) {
                    ...
                    foreach ($ambildatakeranjang as $row) :
                        ...
                        if ($row['jumlah'] > 0) {
                            ...
                            $data4 = [
                                ...
                                'debet'          => 0,
                                'prime_debet'    => 0,
                                'credit'         => $row['jumlah'] * $nilai_tukar * -1,
                                'prime_credit'   => $row['jumlah'] * -1,
                                'keterangan'     => $row['keterangan'],
                                'codejurnal'     => $codejurnal,
                                'rate'           => $nilai_tukar,
                            ];
                    ...
                    $datalog = [
                        ...
                        'kegiatan'  => 'Menambah Bukti Pengeluaran Kas/Bank : ' . $no_bukti,
                    ];
    ...
    public function updatedata()
    {
        if ($this->request->isAJAX()) {
            ...
            $ambildatakeranjang = $this->ModelOthPay->ambilCart($no_bukti);
            ...
            if (empty($ambildatakeranjang)) {
                ...
                foreach ($ambildatakeranjang as $row) :
                    ...
                    if ($stt == 'A') {
                     ...
                     if ($stt == 'D') {
                        $this->ModelOthPay->deleteDetailOthPay($row_id);
                     } 
                     if ($stt == 'E') {
                       $this->ModelOthPay->updateDetailOthPay($datapay);
                     }
                ...
                $datalog = [
                    'username'  => session()->get('username'),
                    'jamtrx'    => date('Y-m-d H:i:s'),
                    'kegiatan'  => 'Merubah Bukti Pengeluaran Kas/Bank : ' . $no_bukti,
                ];
                $this->ModelLogHistory->add($datalog);
    ...
    public function hapus()
    {
        if ($this->request->isAJAX()) {
            ...
            $datalog = [
                'username'  => session()->get('username'),
                'jamtrx'    => date('Y-m-d H:i:s'),
                'kegiatan'  => 'Menghapus Bukti Pengeluaran Kas/Bank : ' . $nobukti,
            ];
            $this->ModelLogHistory->add($datalog);
